...
- (void)logout
{
    NSLog (@"SFSlideNavViewController->logout");
...
- (void)login
{
    NSLog (@"SFSlideNavViewController->login");
...
{
    NSLog (@"SFSlideNavViewController->cellForRowAtIndexPath");
    ...
    if ([indexPath isEqual:self.currentIndex]) {
        ...
        cell.textLabel.textColor = [UIColor whiteColor];//RGBCOLOR(187, 187, 187);
    }
...
{
    NSLog (@"SFSlideNavViewController->didSelectRowAtIndexPath");
...
- (void)viewDidLoad
{
    NSLog (@"SFSlideNavViewController->viewDidLoad");
    [super viewDidLoad];
    ...
    // 设置背景图
    self.slideView.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"left_bg.png"]];
