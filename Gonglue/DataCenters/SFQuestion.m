...
+ (SFQuestionHttpClient *)sharedClient
{
    ...
    dispatch_once(&onceToken, ^{
        _sharedClient = [[SFQuestionHttpClient alloc] initWithBaseURL:[NSURL URLWithString:@"http://gonglue.me/"]];
    });
...
- (void)load
{
    ...
    NSString *url = [NSString stringWithFormat:@"http://gonglue.me/q/%@", self.questionId];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if (self.detailLoadedBlock) {
        self.detailLoadedBlock(questionInfo, 0, [NSError errorWithDomain:@".gonglue.me" code:200 userInfo:nil]);
    }
}
               ...
               withBlock:(SFQuestionDetailLoadedBlock)block
{
    ...
    if (questionInfo) {
        block(questionInfo, 0, [NSError errorWithDomain:@".gonglue.me" code:200 userInfo:nil]);
    }
    else {
                      ...
                      withBlock:(SFQuestionListLoadedBlock)block
{
    ...
    [[SFQuestionHttpClient sharedClient] getPath:path parameters:[NSDictionary dictionaryWithObjectsAndKeys:
                                         ...
                                         success:^(AFHTTPRequestOperation *operation, id JSON) {
                                             ...
                                             if (block) {
                                                 block(data[@"items"], [NSError errorWithDomain:@".gonglue.me" code:status userInfo:nil]);
                                             }
